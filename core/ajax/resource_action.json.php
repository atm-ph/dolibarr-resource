...
if ($actioncode)
	$sql .= " AND ca.code='" . $db->escape($actioncode) . "'";
if ($pid)
...
foreach ( $eventarray as $day => $event_to_send ) {
	...
	if (! empty($event_to_send->fk_project)) {
		...
		// Check if all propal for this project is not signed (if it is the case do not add even to ressource calendar)
		$addevent = true;
		$unsigned = 0;
		$num = 0;
...
$resourcestat = new Resource($db);
$resource_json = array ();
$resourcestat->fetch_all_used('ASC', 't.rowid', 0, 0, array (
		't.resource_type' => 'building@place' 
));
